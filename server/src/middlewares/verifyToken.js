...
export const verifyToken = async (req, res, next) => {
    ...
    if (!authHeader) {
        ...
        if (!refreshToken) {
            return res.status(401).json({ success: "fail", message: "Unauthorized user" });
        }
        ...
        try {
            ...
            return res.status(error.statusCode || 401).json({ success: "fail", message: error.message || "Unauthorized user" });
        }
    }
